...
#define BOOT_PIN					 PIN_PB22 //pin tied to button for stay in boot mode
// spi flash stuff
	...
	*/
	...
	while (APP_START_ADDRESS + addr_i < 0x40000) {	
		...
		for (int i = 0; i < 4; i++) {
			...
			at25dfx_chip_read_buffer(&at25dfx_chip, thisFlashAddr, read_buffer, NVMCTRL_PAGE_SIZE); // read 64B from SPI flash
